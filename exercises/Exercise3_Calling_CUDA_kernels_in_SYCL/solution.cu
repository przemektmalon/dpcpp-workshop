...
int main(int argc, char *argv[]) {
...

  {
    ...
    myQueue.submit([&](sycl::handler &h) {
      ...
      auto accA = bA.get_access<sycl::access::mode::read>(h);
      auto accB = bB.get_access<sycl::access::mode::read>(h);
      auto accC = bC.get_access<sycl::access::mode::write>(h);

      h.interop_task([=](sycl::interop_handler ih) {
        auto dA =
            reinterpret_cast<double *>(ih.get_mem<sycl::backend::cuda>(accA));
        auto dB =
            reinterpret_cast<double *>(ih.get_mem<sycl::backend::cuda>(accB));
        auto dC =
            reinterpret_cast<double *>(ih.get_mem<sycl::backend::cuda>(accC));
